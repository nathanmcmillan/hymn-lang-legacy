...
func (me *parser) trying() (*node, *parseError) {
	...
	if !ok || len(enfunc.types) != 2 {
		er := err(me, ECodeEnumParameter, "`try` requires the current function to return an enum with exactly two types")
		return nil, er
	}
	...
	if !ok || len(encalc.types) != 2 {
		er := err(me, ECodeEnumParameter, "`try` requires the following expression to return an enum with exactly two types")
		return nil, er
	}
	...
	data, er := encalc.getuniondata(me.hmfile, encalc.types[1].name)
	if er != nil {
		return nil, er
	}
